var ajaxurl = 'http://wpmegamenu.com/wp-admin/admin-ajax.php';
window._wpemojiSettings = {
    "baseUrl": "http:\/\/s.w.org\/images\/core\/emoji\/72x72\/",
    "ext": ".png",
    "source": {
        "concatemoji": "http:\/\/wpmegamenu.com\/wp-includes\/js\/wp-emoji-release.min.js?ver=4.2.2"
    }
};
! function(a, b, c) {
    function d(a) {
...
jQuery(document).ready(function($) {
    ...
    var skin_prefix = 'crumenu-skin-';
    var trans_prefix = 'crumenu-transition-';
    var $menu = $('.crumenu-main, .crumenu-responsive-toggle-main');
    $('.crumenu-demo-switcher').on('click', '.crumenu-demo-skin', function() {
        $menu.removeClass(skin_prefix + current_skin);
        $(this).siblings().removeClass('crumenu-demo-op-selected');
        $(this).addClass('crumenu-demo-op-selected');
        current_skin = $(this).data('skin');
        $menu.addClass(skin_prefix + current_skin);
    });
    $('.crumenu-demo-switcher').on('click', '.crumenu-demo-transition', function() {
    ...
    $('.crumenu-demo-tabs').on('click', 'h5', function() {
        $(this).siblings().removeClass('demo-tab-active');
        $(this).addClass('demo-tab-active');
        $('.crumenu-demo-tab-panels').find('.crumenu-demo-tab-panel').hide();
        $('#' + $(this).data('target')).fadeIn();
    });
    $('.crumenu-demo-tabs > h5:first').click();
});
var crumenu_data = {
    "remove_conflicts": "on",
    ...
    "touch_off_close": "on",
    "v": "3.2.0.2",
    "ajax_url": "http:\/\/wpmegamenu.com\/wp-admin\/admin-ajax.php"
};
'use strict';
(function(d, f, h, k) {
    function m(a, c) {
...
var crumenu_sticky_settings = [];
'use strict';
jQuery(document).ready(function(b) {
    if (crumenu_sticky_settings)
        for (var a in crumenu_sticky_settings)
            if (crumenu_sticky_settings.hasOwnProperty(a)) {
                var f = 1 == crumenu_sticky_settings[a].is_sticky ? !0 : !1,
                    g = b(".crumenu-" + a),
                    e = b(".crumenu-responsive-toggle-" + a);
                g.find("> .crumenu-nav");
                g.add(e);
                1 == crumenu_sticky_settings[a].is_mobile && g.addClass("crumenu-is-mobile");
                if (f) {
                    if (f = "on" == crumenu_sticky_settings[a].full_width_menu_bar ? !0 : !1) g.addClass("crumenu-sticky-full-width"), e.addClass("crumenu-sticky-full-width");
                    if (!f) {
                        var l = g.filter(".crumenu-bar-align-full");
                        l.each(function() {
                    ...
                    crumenu_sticky_settings[a].permanent || (f = crumenu_sticky_settings[a].sticky_offset ? crumenu_sticky_settings[a].sticky_offset : 0, g.sticky({
                        topSpacing: f,
                        className: "crumenu-sticky",
                        wrapperClassName: "crumenu-sticky-wrapper"
                    }), g.on("sticky-start sticky-end", function() {
                        setTimeout(function() {
                            g.crumenu("positionSubmenus")
                        }, 100)
                    }), e.sticky({
                        topSpacing: f,
                        className: "crumenu-sticky",
                        wrapperClassName: "crumenu-sticky-toggle-wrapper"
                    }))
                }
            }
});
    ...
    var a = {
        ...
        p = function() {
            for (var b = f.scrollTop(), h = g.height(), n = h - l, n = b > n ? n - b : 0, d = 0; d < e.length; d++) {
                ...
                b <= a ? null !== c.currentTop && (c.stickyElement.css("position", "").css("top", ""), c.stickyElement.removeClass(c.className), c.stickyElement.trigger("sticky-end", [c]).parent().removeClass(c.className), c.currentTop = null) : (a = h - c.stickyElement.outerHeight() - c.topSpacing - c.bottomSpacing - b - n, a = 0 > a ? a + c.topSpacing : c.topSpacing, c.currentTop != a && (c.stickyElement.css("position", "fixed").css("top", a), c.stickyElement.trigger("sticky-start", [c]).parent().addClass(c.className), c.stickyElement.addClass(c.className), c.currentTop = a))
            }
        }, q = function() {
            l = f.height()
        }, k = {
            init: function(m) {
                ...
                return this.each(function() {
                    var a = b(this),
                        d = a.attr("id");
                    d || (d = a.data("crumenu-target") + "-toggle");
                    d = b("<div></div>").attr("id", d + "-sticky-wrapper").addClass(h.wrapperClassName);
                    a.wrapAll(d);
            ...
            unstick: function(a) {
                return this.each(function() {
                    for (var a = b(this), m = -1, d = 0; d < e.length; d++) e[d].stickyElement.get(0) == a.get(0) && (m = d); - 1 != m && (e.splice(m, 1), a.unwrap(), a.removeAttr("style"))
                })
            }
        };
...
var _wpcf7 = {
    "loaderUrl": "http:\/\/wpmegamenu.com\/wp-content\/plugins\/contact-form-7\/images\/ajax-loader.gif",
    "sending": "Sending ...",
    "cached": "1"
};
(function($) {
